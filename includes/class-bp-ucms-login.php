...
final class BP_UCMS_Login {
    ...
    public function capture_login_redirect_target($user, string $username, string $password) {
        ...
        if (!empty($_SERVER['HTTP_REFERER'])) {
            ...
            if (
                ...
                strpos($referer, $login_url) === false &&
                strpos($referer, $custom_login_base) === false
            ) {
    ...
    public function handle_login_redirect(string $redirect_to, string $requested_redirect_to, $user): string {
        ...
        $mode = $settings['redirect_' . $role . '_mode'] ?? '';
        $page_id = !empty($settings['redirect_' . $role . '_page_id']) ? (int) $settings['redirect_' . $role . '_page_id'] : 0;

        // No redirect selected: do not override other plugins or WordPress behavior.
        if ($mode === '') {
            return $redirect_to;
        }
        ...
        if (
            ...
            $this->is_safe_frontend_url($requested_redirect_to)
        ) {
        ...
        if (
            ...
            $this->is_safe_frontend_url(wp_unslash($_REQUEST['redirect_to']))
        ) {
