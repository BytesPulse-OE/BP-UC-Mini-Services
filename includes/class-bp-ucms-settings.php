...
final class BP_UCMS_Settings {
    ...
    public function defaults(): array {
        return [
            ...
            'redirect_administrator_mode' => 'same',
            'redirect_administrator_page_id' => 0,
            'redirect_editor_mode' => 'same',
            'redirect_editor_page_id' => 0,
            'redirect_author_mode' => 'same',
            'redirect_author_page_id' => 0,
            'redirect_contributor_mode' => 'same',
            'redirect_contributor_page_id' => 0,
            'redirect_subscriber_mode' => 'same',
            'redirect_subscriber_page_id' => 0,
            'redirect_customer_mode' => 'same',
            'redirect_customer_page_id' => 0,
            'redirect_client_mode' => 'same',
    ...
    public function import_from_array(array $import): array {
        ...
        foreach ($mode_fields as $field) {
            $value = sanitize_key((string) ($merged[$field] ?? 'same'));
            $merged[$field] = in_array($value, ['same', 'page', 'admin'], true) ? $value : 'same';
        }
